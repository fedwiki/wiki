  ...
  var farmServ = http.createServer(function(req, res) {
    ...
    if (hosts[incHost]) {
      ...
      if (!allow(incHost)) {
        ...
        res.statusCode = 400;
        const errorText = errorPage.render('Requested Wiki Does Not Exist','The wiki you are trying to access does not exist.',`You may visit <a href='//${upHost}'>${upHost}</a> for more information.`);
        res.end(errorText);
        return;
      }
